...
**Internal validity.** The simulated benchmarking controlled for human variability through the Oracle protocol, but the stratification of the Nutrition5k subset into Simple and Complex categories relied on researcher judgement operationalised through a multi-factor heuristic scoring engine. A systematic audit of this engine, conducted as part of the threshold calibration process, revealed that the initial classification threshold ($\theta_s = 0.50$) was calibrated under an implicit assumption of score uniformity that did not hold in practice: empirical analysis of the full 5,006-dish corpus revealed a heavily left-skewed distribution (mean = 0.194, max = 0.686), situating the initial boundary at the 97th percentile and classifying only 3.7% of dishes as Complex. The audit additionally identified keyword-level heuristic failures — most notably, the generic term "mixed" triggering simultaneous inflation of both the visual distinctiveness and ambiguity scores for ingredient names such as "mixed greens" that are nutritionally unambiguous. Corrective recalibration (threshold lowered to $\theta_s = 0.35$; keyword and default-score fixes applied) was applied before the threshold sweep was executed, raising the Complex proportion to 27.0% and restoring sufficient statistical power to estimate the True Positive Rate. The fact that this miscalibration was discovered \emph{during} the evaluation process rather than prior to it underscores the circularity inherent in calibrating a stratification instrument on the corpus used to evaluate the system it stratifies. Alternative stratification criteria, or independent expert dietary annotation, could yield materially different Simple/Complex partitions and correspondingly different performance profiles. The threshold calibration itself was additionally performed on the same dataset used for evaluation, introducing a further risk of overfitting the routing parameters to the specific characteristics of the Nutrition5k corpus.
